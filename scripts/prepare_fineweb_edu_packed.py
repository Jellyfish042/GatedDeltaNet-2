...
def existing_chunk_count(outdir: Path, prefix: str, chunk_size: int) -> int:
    ...
    if indices != expected:
        raise ValueError(f"Packed shards for {prefix!r} are not contiguous from 0: {indices[:5]} ... {indices[-5:]}")
    return len(indices)
...
def prepare_builder(
    ...
    vocab_size: int,
    resume: bool,
) -> tuple[PackedDatasetBuilder, int]:
    outdir.mkdir(parents=True, exist_ok=True)
    count = existing_chunk_count(outdir, prefix, chunk_size)
    if count and not resume:
        raise FileExistsError(f"{outdir} already has {count} packed shard(s). Use --resume or clean the output directory.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare FineWeb-Edu as LitGPT packed .bin shards.")
    parser.add_argument("--out-root", type=Path, required=True)
    parser.add_argument("--dataset", default="HuggingFaceFW/fineweb-edu")
    parser.add_argument("--config", default="sample-100BT")
    parser.add_argument("--split", default="train")
    parser.add_argument("--tokenizer", default="TinyLlama/TinyLlama_v1.1")
    parser.add_argument("--train-tokens", type=parse_tokens, default=parse_tokens("10B"))
    parser.add_argument("--val-tokens", type=parse_tokens, default=parse_tokens("50M"))
    parser.add_argument("--train-dir-name", default="train_10B")
    parser.add_argument("--val-dir-name", default="val_50M")
    parser.add_argument("--manifest-name", default="fineweb_edu_manifest.json")
    ...
    train_chunk_size = args.train_block_size * args.train_chunk_blocks
    val_chunk_size = args.val_block_size * args.val_chunk_blocks
    val_builder, committed_val_tokens = prepare_builder(
        val_dir, "validation", val_chunk_size, eos_id, vocab_size, args.resume
    )
    train_builder, committed_train_tokens = prepare_builder(
        train_dir, "train_slim", train_chunk_size, eos_id, vocab_size, args.resume
    )

    if committed_val_tokens > args.val_tokens or committed_train_tokens > args.train_tokens:
        raise ValueError(
            "Existing committed token count exceeds requested target. "
            f"val={committed_val_tokens}/{args.val_tokens}, train={committed_train_tokens}/{args.train_tokens}",
        )
    ...
    def checkpoint(status: str) -> None:
        ...
        write_manifest(
            manifest_path,
            {
                ...
                "tokens_per_second_this_run": (total - committed_val_tokens - committed_train_tokens) / elapsed,
    ...
    def consume(texts: list[str]) -> None:
        nonlocal docs, train_tokens, val_tokens, skip_val_tokens, skip_train_tokens
        ...
        for ids in encoded:
            ...
            if docs % 10000 == 0:
                ...
                print(
                    ...
                    f"new_rate={(total - committed_val_tokens - committed_train_tokens) / elapsed:,.0f} tok/s",
                    flush=True,
                )
                checkpoint("running")
    ...
    for row in dataset:
        ...
        if len(batch) >= args.batch_size:
            consume(batch)
            batch = []
    if batch and (train_tokens < args.train_tokens or val_tokens < args.val_tokens):
        consume(batch)
